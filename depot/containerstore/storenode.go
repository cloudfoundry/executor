...
func (n *storeNode) Create(logger lager.Logger) error {
	...
	fmt.Fprintf(logStreamer.Stdout(), "Cell %s creating container\n", n.cellID)
	gardenContainer, err := n.createGardenContainer(logger, &info)
	if err != nil {
		logger.Error("failed-to-create-container", err)
		fmt.Fprintf(logStreamer.Stderr(), "Cell %s failed to create container: %s\n", n.cellID, err.Error())
		n.complete(logger, true, truncatedErrorMessage("%s: %s", ContainerInitializationFailedMessage, err.Error()))
		return err
	}
	fmt.Fprintf(logStreamer.Stdout(), "Cell %s successfully created container\n", n.cellID)
...
func (n *storeNode) Destroy(logger lager.Logger) error {
	...
	fmt.Fprintf(logStreamer.Stdout(), "Cell %s destroying container\n", n.cellID)

	err = n.destroyContainer(logger)
	if err != nil {
		fmt.Fprintf(logStreamer.Stdout(), "Cell %s failed to destroy container\n", n.cellID)
		return err
	}
	fmt.Fprintf(logStreamer.Stdout(), "Cell %s successfully destroyed container\n", n.cellID)
