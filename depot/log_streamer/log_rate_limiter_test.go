...
var _ = Describe("LogRateLimiter", func() {
	It("limits logs by byte", func() {
		...
		Expect(logRateLimiter.Limit("test", map[string]string{}, 1)).ToNot(HaveOccurred())
		Expect(logRateLimiter.Limit("test", map[string]string{}, 1)).To(MatchError("app instance exceeded log rate limit (5 bytes/sec)"))
	})
	It("limits logs by line", func() {
		...
		Expect(logRateLimiter.Limit("test", map[string]string{}, 1)).ToNot(HaveOccurred())
		Expect(logRateLimiter.Limit("test", map[string]string{}, 1)).To(MatchError("app instance exceeded log rate limit (1 log-lines/sec) set by platform operator"))
	})
	It("cannot log if log limit by byte is 0", func() {
		...
		Expect(logRateLimiter.Limit("test", map[string]string{}, 1)).To(MatchError("Not allowed to log"))
	})
	It("send the over limit message to loggregator", func() {
		...
		Expect(logRateLimiter.Limit("test", map[string]string{}, 1)).To(HaveOccurred())
