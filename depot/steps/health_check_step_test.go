	...
	Describe("Run", func() {
		...
		Context("when the readiness check fails", func() {
			...
			It("completes with failure", func() {
				...
				emittableError, ok := err.(*steps.EmittableError)
				Expect(ok).To(BeTrue())
				Expect(emittableError.WrappedError()).To(MatchError(ContainSubstring("booom!")))
			})
		...
		Context("when the readiness check passes", func() {
			...
			Context("and the liveness check fails", func() {
				...
				It("completes with failure", func() {
					...
					emittableError, ok := err.(*steps.EmittableError)
					Expect(ok).To(BeTrue())
					Expect(emittableError.WrappedError()).To(Equal(disaster))
				})
			})
		})
	})
