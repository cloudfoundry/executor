...
func (t *transformer) transformCheckDefinition(
	...
	createCheck := func(path string, port, timeout int, http, readiness bool, interval time.Duration) models.ActionInterface {
		...
		if readiness {
			args = append(args, fmt.Sprintf("-readiness=%dns", interval))
		} else {
			args = append(args, fmt.Sprintf("-liveness=%dns", interval))
		}
	...
	for _, check := range container.CheckDefinition.Checks {
		if err := check.Validate(); err != nil {
			...
			livenessChecks = append(livenessChecks, createCheck(path, int(check.HttpCheck.Port), timeout, true, false, t.unhealthyMonitoringInterval))
			...
			livenessChecks = append(livenessChecks, createCheck("", int(check.TcpCheck.Port), timeout, false, false, t.unhealthyMonitoringInterval))
		}
	}
